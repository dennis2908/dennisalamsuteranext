...
const columns = [
  ...
  { id: 'qty', label: 'Qty', align: 'right',minWidth: 170 },
  {
    id: 'code',
    label: 'Code',
    minWidth: 100,
...
export default function Barang_list() {
  ...
  const [DialogSEtitle, setDialogSEtitle] = useStateRef("New Data Barang");
  ...
  const delItem = async() => {
	  await fetch("http://127.0.0.1:8441/api/barang/"+FormData.id, {
      method: "DELETE",
      headers: {"Authorization" : "Bearer "+storeLogin.getState().authLogin}
				}).then(res => res.json())
			  .then(
  ...
  const saveUpdateData = (e) => {
	  ...
	  fetch("http://127.0.0.1:8441/api/barang", {
						  method: "POST",
						  headers: {
		  ...
		  fetch("http://127.0.0.1:8441/api/barang/"+FormData.id, {
						  method: "PUT",
						  headers: {
							  'Accept': 'application/json',
							  'Content-Type': 'application/json',
  ...
  const handleClickOpen = (data) => {
	setOpen(true);
	setFormData(data);
	setDialogSEtitle("Edit Data Barang")
  ...
  const openNewForm = () => {
	...
	setDialogSEtitle("New Data Barang")
	console.log("New Data Barang")
	setBtnDilSE('Save')
  };
      ...
      <Row>
        <Col breakPoint={{ xs: 24, md: 12 }}>
          <Card status="Primary" accent="Info">
            <CardHeader><ListIcon style={{marginBottom:-7}} color="primary"/> DATA BARANG <Button 
			style={{marginLeft:30}}
			onClick={() => openNewForm()}
          ...
          <form autoComplete="off" onSubmit={(e) => saveUpdateData(e)}>
			  <TextField
				...
				<TextField
				  ...
				  fullWidth
				  style={{marginBottom:20}}
				  onChange={onFieldChange('qty').bind(this)}
				/>
				<TextField
				  required
				  type="number"
				  ...
				  fullWidth
				  style={{marginBottom:20}}
				  onChange={onFieldChange('price').bind(this)}
				/>
				<TextField
				  required
				  label="Description"
				  ...
				  fullWidth
				  style={{marginBottom:20}}
				  onChange={onFieldChange('desc').bind(this)}
				/>
