...
export default function User_list() {
  ...
  const [DialogSEtitle, setDialogSEtitle] = useStateRef("New Data User");
  ...
  const handleClickOpen = (data) => {
    setOpen(true);
    setFormData(data);
    setDialogSEtitle("Edit Data User")
  ...
  const openNewForm = () => {
    ...
    setDialogSEtitle("New Data User")
  ...
  let dataSelRole =   [];
  dataSelRole =   [<option value='' key={String("0role") + String("0role")}>== Pilih Role ==</option>];
	  if(selrole){
		  selrole.forEach((k, v) => {
        dataSelRole.push(<option value={k.id} key={String(k.id+"role")}>{k.role_name}</option>);	
		  })
	  }
  return (
      ...
      <Row>
        <Col breakPoint={{ xs: 24, md: 12 }}>
          <Card status="Primary" accent="Info">
            <CardHeader><ListIcon style={{marginBottom:-7}} color="primary"/>DATA USER<Button 
			style={{marginLeft:30}}
			onClick={() => openNewForm()}
          ...
          <form autoComplete="off" onSubmit={(e) => saveUpdateData(e)}>
			  <TextField
				...
				<TextField
				  ...
				  fullWidth
				  style={{marginBottom:20}}
				  onChange={onFieldChange('username').bind(this)}
				/>
        <TextField
          required
          type="email"
