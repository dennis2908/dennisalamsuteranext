...
const columns = [
  ...
  { id: 'address', align: 'left',label: 'Address', minWidth: 270 },
];
...
export default function Customer_list() {
  ...
  const [DialogSEtitle, setDialogSEtitle] = useStateRef("New Data Customer");
  ...
  const loadData = async(e) => {
    await DoShowLin()
	  await fetch("https://alamsuteradennis.herokuapp.com/api/customer", {
      method: "GET",
  ...
  const delItem = async() => {
	  await fetch("https://alamsuteradennis.herokuapp.com/api/customer/"+FormData.id, {
      method: "DELETE",
      headers: {"Authorization" : "Bearer "+storeLogin.getState().authLogin}
				}).then(res => res.json())
			  .then(
  ...
  const saveUpdateData = (e) => {
	  ...
	  fetch("https://alamsuteradennis.herokuapp.com/api/customer", {
						  method: "POST",
						  headers: {
		  ...
		  fetch("https://alamsuteradennis.herokuapp.com/api/customer/"+FormData.id, {
						  method: "PUT",
						  headers: {
							  'Accept': 'application/json',
							  'Content-Type': 'application/json',
  ...
  const handleClickOpen = (data) => {
	setOpen(true);
	setFormData(data);
	setDialogSEtitle("Edit Data Customer")
  ...
  const openNewForm = () => {
	...
	setDialogSEtitle("New Data Customer")
	console.log("New Data Customer")
	setBtnDilSE('Save')
  };
  ...
  return (
    <Layout title="List Customer">
      <Row>
        <Col breakPoint={{ xs: 24, md: 12 }}>
          <Card status="Primary" accent="Info">
            <CardHeader><ListIcon style={{marginBottom:-7}} color="primary"/> DATA CUSTOMER <Button 
			style={{marginLeft:30}}
			onClick={() => openNewForm()}
          ...
          <form autoComplete="off" onSubmit={(e) => saveUpdateData(e)}>
			  <TextField
				...
				<TextField
				  ...
				  fullWidth
				  style={{marginBottom:20}}
				  onChange={onFieldChange('code').bind(this)}
				/>
				<TextField
				  required
				  name="phone"
				  ...
				  fullWidth
				  style={{marginBottom:20}}
				  onChange={onFieldChange('phone').bind(this)}
				/>
				<TextField
				  required
				  name="address"
				  ...
				  fullWidth
				  style={{marginBottom:20}}
				  onChange={onFieldChange('address').bind(this)}
				/>
